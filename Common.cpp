...
/// 通过DMatch,获取特征点,按照配对顺序放入容器
/// \param imgpts1 前一张
/// \param imgpts2 后一张
/// \param matches ;是已经匹配好的特征点
/// \param pt_set1;承接作用
/// \param pt_set2
...
///
/// \param dir_name  图片文件夹
/// \param images	承接用的image vector
/// \param images_names 存图片名的vector
...
//Qin		if (files_[i][0] == '.' || !(hasEndingLower(files_[i],"jpg")||hasEndingLower(files_[i],"png"))) {
//			continue;
//		}
        img_dir= "/home/qin/opencv_practice/Chapter4_StructureFromMotion/img/";
		//cv::Mat m_ = cv::imread(string(dir_name_).append("/").append(files_[i]));//append()添加文本的方法
		int j=i+1;

		 ss= std::to_string(j);
		 img_dir+= ss+".jpg";
		 file_name=ss+".jpg";
		 m_ =cv::imread(img_dir);
		cout<<m_.size()<<endl;
		cout<<file_name<<endl;
		cout<<ss<<endl;
