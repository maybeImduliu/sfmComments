					  ...
					  ) 
{
	...
	Mat F;
	{
		...
		//核心函数，计算F，并得到优化的匹配组模板status,status是在计算过程中没有被舍弃的点，元素被被置为1；否则置为0。
...
bool TestTriangulation(const vector<CloudPoint>& pcloud, const Matx34d& P, vector<uchar>& status) {
	...
	for(int i=0;i<12;i++) P4x4.val[i] = P.val[i];//复制
	
	perspectiveTransform(pcloud_pt3d, pcloud_pt3d_projected, P4x4); //这里是应用P对三维点做透视变换
	
	status.resize(pcloud.size(),0);
	for (int i=0; i<pcloud.size(); i++) {
		status[i] = (pcloud_pt3d_projected[i].z > 0) ? 1 : 0;   //判断z坐标是否在摄像机前面,
	}
	int count = countNonZero(status);//计数，得到在摄像机前面点的个数

	double percentage = ((double)count / (double)pcloud.size());//摄像机前面的点在总点数中的占比，少于75%就代表P1无效
	...
	Mat_<double>& t2) 
{
	...
	Matx33d W(0,-1,0,	//HZ 9.13,顺时针旋转90
		1,0,0,
		0,0,1);
	Matx33d Wt(0,1,0,//逆时针旋转90
		-1,0,0,
		0,0,1);
	R1 = svd_u * Mat(W) * svd_vt; //HZ 9.19
	R2 = svd_u * Mat(Wt) * svd_vt; //HZ 9.19
	t1 = svd_u.col(2); //u3,u3这列直接取出来就是位移?
	t2 = -svd_u.col(2); //u3
#else
						...
						) 
{
	//Find camera matrices
	{
		...
		//decompose E to P' , HZ (9.19),分解本质矩阵到旋转和位移
		{			
			...
			double reproj_error2 = TriangulatePoints(imgpts2_good, imgpts1_good, K, Kinv, distcoeff, P1, P, pcloud1, corresp);//交叉检验?
			vector<uchar> tmp_status;

			//check if pointa are triangulated --in front-- of cameras for all 4 ambiguations,,P就是世界坐标系
			if (!TestTriangulation(pcloud,P1,tmp_status) || !TestTriangulation(pcloud1,P,tmp_status) || reproj_error1 > 100.0 || reproj_error2 > 100.0) {

			    //进入这条分支表示重投影误差比较大,或者投影在正前方的点过少;重新再来一遍,算的结果有可能不一样?
