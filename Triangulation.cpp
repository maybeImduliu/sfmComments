...
Mat_<double> IterativeLinearLSTriangulation(Point3d u,	//homogenous image point (u,v,1)
	...
	Mat_<double> X_ = LinearLSTriangulation(u,P,u1,P1);//这次计算是没有权重的
...
//Triagulate points
/// 三角化,返回值是重投影误差
/// \param pt_set1,两个特征点集
/// \param pt_set2
/// \param K
/// \param Kinv
/// \param distcoeff
/// \param P,两个相机位姿,其中一个一般选择标准的世界坐标系
/// \param P1
/// \param pointcloud,承接3d点的容器
/// \param correspImg1Pt
/// \return
...
#else
	Mat_<double> KP1 = K * Mat(P1);//内外参乘积,用来计算重投影误差
#pragma omp parallel for num_threads(1)
	for (int i=0; i<pts_size; i++) {
		Point2f kp = pt_set1[i].pt; 
		Point3d u(kp.x,kp.y,1.0);
		Mat_<double> um = Kinv * Mat_<double>(u); //k逆乘以齐次坐标系. 2d=K*3d;所以3d=K逆*2d,算出来是在当前坐标系的相机坐标
		...
		Mat_<double> X = IterativeLinearLSTriangulation(u,P,u1,P1);//X是齐次的3d坐标
...
//		cout <<	"Point * K: " << xPt_img << endl;
		Point2f xPt_img_(xPt_img(0)/xPt_img(2),xPt_img(1)/xPt_img(2));  //再除以z;
				
#pragma omp critical //单线程
		{
			double reprj_err = norm(xPt_img_-kp1); //计算重投影误差
			reproj_error.push_back(reprj_err);

			CloudPoint cp; //这个结构体有三个东西,点\重投影误差\归哪张图
			cp.pt = Point3d(X(0),X(1),X(2));
			cp.reprojection_error = reprj_err;
			
			pointcloud.push_back(cp);
			correspImg1Pt.push_back(pt_set1[i]);//记录重投影误差对应哪两帧的特征点
#ifdef __SFM__DEBUG__
